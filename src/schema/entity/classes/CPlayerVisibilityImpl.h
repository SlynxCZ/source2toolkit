...
#ifndef _INCLUDE_CPLAYERVISIBILITYIMPL_H
#define _INCLUDE_CPLAYERVISIBILITYIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IPlayerVisibility.h"
...
public:
    explicit CPlayerVisibilityImpl(CPlayerVisibility* p) : CBaseEntityImpl(p) {}

private:
    CPlayerVisibility* Real() { return static_cast<CPlayerVisibility*>(m_pReal); }
    CPlayerVisibility* Real() const { return static_cast<CPlayerVisibility*>(m_pReal); }

public:
    CPlayerVisibility* GetOriginal() const override { return Real(); }
    float& VisibilityStrength() override { return Real()->m_flVisibilityStrength(); }
    void VisibilityStrengthUpdated() override { Real()->m_flVisibilityStrength.NetworkStateChanged(); }
    float& FogDistanceMultiplier() override { return Real()->m_flFogDistanceMultiplier(); }
    void FogDistanceMultiplierUpdated() override { Real()->m_flFogDistanceMultiplier.NetworkStateChanged(); }
    float& FogMaxDensityMultiplier() override { return Real()->m_flFogMaxDensityMultiplier(); }
    void FogMaxDensityMultiplierUpdated() override { Real()->m_flFogMaxDensityMultiplier.NetworkStateChanged(); }
