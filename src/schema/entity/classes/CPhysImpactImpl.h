...
#ifndef _INCLUDE_CPHYSIMPACTIMPL_H
#define _INCLUDE_CPHYSIMPACTIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IPhysImpact.h"
...
public:
    explicit CPhysImpactImpl(CPhysImpact* p) : CPointEntityImpl(p) {}

private:
    CPhysImpact* Real() { return static_cast<CPhysImpact*>(m_pReal); }
    CPhysImpact* Real() const { return static_cast<CPhysImpact*>(m_pReal); }

public:
    CPhysImpact* GetOriginal() const override { return Real(); }
    float& Damage() override { return Real()->m_damage(); }
    void DamageUpdated() override { Real()->m_damage.NetworkStateChanged(); }
    float& Distance() override { return Real()->m_distance(); }
    void DistanceUpdated() override { Real()->m_distance.NetworkStateChanged(); }
    CUtlSymbolLarge& DirectionEntityName() override { return Real()->m_directionEntityName(); }
