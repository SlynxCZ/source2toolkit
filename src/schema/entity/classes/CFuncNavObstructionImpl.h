...
#ifndef _INCLUDE_CFUNCNAVOBSTRUCTIONIMPL_H
#define _INCLUDE_CFUNCNAVOBSTRUCTIONIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IFuncNavObstruction.h"
...
public:
    explicit CFuncNavObstructionImpl(CFuncNavObstruction* p) : CBaseModelEntityImpl(p) {}

private:
    CFuncNavObstruction* Real() { return static_cast<CFuncNavObstruction*>(m_pReal); }
    CFuncNavObstruction* Real() const { return static_cast<CFuncNavObstruction*>(m_pReal); }

public:
    CFuncNavObstruction* GetOriginal() const override { return Real(); }
    bool& Disabled() override { return Real()->m_bDisabled(); }
    void DisabledUpdated() override { Real()->m_bDisabled.NetworkStateChanged(); }
    bool& UseAsyncObstacleUpdate() override { return Real()->m_bUseAsyncObstacleUpdate(); }
