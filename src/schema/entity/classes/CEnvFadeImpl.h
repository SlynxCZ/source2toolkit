...
#ifndef _INCLUDE_CENVFADEIMPL_H
#define _INCLUDE_CENVFADEIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IEnvFade.h"
...
public:
    explicit CEnvFadeImpl(CEnvFade* p) : CLogicalEntityImpl(p) {}

private:
    CEnvFade* Real() { return static_cast<CEnvFade*>(m_pReal); }
    CEnvFade* Real() const { return static_cast<CEnvFade*>(m_pReal); }

public:
    CEnvFade* GetOriginal() const override { return Real(); }
    Color& FadeColor() override { return Real()->m_fadeColor(); }
    void FadeColorUpdated() override { Real()->m_fadeColor.NetworkStateChanged(); }
