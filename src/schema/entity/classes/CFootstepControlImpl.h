...
#ifndef _INCLUDE_CFOOTSTEPCONTROLIMPL_H
#define _INCLUDE_CFOOTSTEPCONTROLIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IFootstepControl.h"
...
public:
    explicit CFootstepControlImpl(CFootstepControl* p) : CBaseTriggerImpl(p) {}

private:
    CFootstepControl* Real() { return static_cast<CFootstepControl*>(m_pReal); }
    CFootstepControl* Real() const { return static_cast<CFootstepControl*>(m_pReal); }

public:
    CFootstepControl* GetOriginal() const override { return Real(); }
    CUtlSymbolLarge& Source() override { return Real()->m_source(); }
    void SourceUpdated() override { Real()->m_source.NetworkStateChanged(); }
    CUtlSymbolLarge& Destination() override { return Real()->m_destination(); }
