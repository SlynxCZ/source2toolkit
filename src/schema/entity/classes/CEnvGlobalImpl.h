...
#ifndef _INCLUDE_CENVGLOBALIMPL_H
#define _INCLUDE_CENVGLOBALIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IEnvGlobal.h"
...
public:
    explicit CEnvGlobalImpl(CEnvGlobal* p) : CLogicalEntityImpl(p) {}

private:
    CEnvGlobal* Real() { return static_cast<CEnvGlobal*>(m_pReal); }
    CEnvGlobal* Real() const { return static_cast<CEnvGlobal*>(m_pReal); }

public:
    CEnvGlobal* GetOriginal() const override { return Real(); }
    ...
    int32_t& Triggermode() override { return Real()->m_triggermode(); }
    void TriggermodeUpdated() override { Real()->m_triggermode.NetworkStateChanged(); }
    int32_t& Initialstate() override { return Real()->m_initialstate(); }
    void InitialstateUpdated() override { Real()->m_initialstate.NetworkStateChanged(); }
    int32_t& Counter() override { return Real()->m_counter(); }
