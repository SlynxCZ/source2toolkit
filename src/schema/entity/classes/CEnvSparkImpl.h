...
#ifndef _INCLUDE_CENVSPARKIMPL_H
#define _INCLUDE_CENVSPARKIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IEnvSpark.h"
...
public:
    explicit CEnvSparkImpl(CEnvSpark* p) : CPointEntityImpl(p) {}

private:
    CEnvSpark* Real() { return static_cast<CEnvSpark*>(m_pReal); }
    CEnvSpark* Real() const { return static_cast<CEnvSpark*>(m_pReal); }

public:
    CEnvSpark* GetOriginal() const override { return Real(); }
    float& Delay() override { return Real()->m_flDelay(); }
    void DelayUpdated() override { Real()->m_flDelay.NetworkStateChanged(); }
    int32_t& Magnitude() override { return Real()->m_nMagnitude(); }
    void MagnitudeUpdated() override { Real()->m_nMagnitude.NetworkStateChanged(); }
    int32_t& TrailLength() override { return Real()->m_nTrailLength(); }
