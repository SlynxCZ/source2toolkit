...
#ifndef _INCLUDE_CFUNCNAVBLOCKERIMPL_H
#define _INCLUDE_CFUNCNAVBLOCKERIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IFuncNavBlocker.h"
...
public:
    explicit CFuncNavBlockerImpl(CFuncNavBlocker* p) : CBaseModelEntityImpl(p) {}

private:
    CFuncNavBlocker* Real() { return static_cast<CFuncNavBlocker*>(m_pReal); }
    CFuncNavBlocker* Real() const { return static_cast<CFuncNavBlocker*>(m_pReal); }

public:
    CFuncNavBlocker* GetOriginal() const override { return Real(); }
    bool& Disabled() override { return Real()->m_bDisabled(); }
    void DisabledUpdated() override { Real()->m_bDisabled.NetworkStateChanged(); }
    int32_t& BlockedTeamNumber() override { return Real()->m_nBlockedTeamNumber(); }
