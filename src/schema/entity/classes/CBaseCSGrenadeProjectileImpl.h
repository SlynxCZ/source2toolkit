...
class CBaseCSGrenadeProjectileImpl : public CBaseGrenadeImpl, public virtual IBaseCSGrenadeProjectile
{
...
private:
    ...
    CBaseCSGrenadeProjectile* Real() const { return static_cast<CBaseCSGrenadeProjectile*>(m_pReal); }

public:
    CBaseCSGrenadeProjectile* GetOriginal() const override { return Real(); }
...
inline IBaseCSGrenadeProjectile* CBaseCSGrenadeProjectile::ToInterface()
{
    static const char s_tag = 0;
    auto& byTag = virtualhooks::entityInterfaces[this];
    auto tagIt = byTag.find(&s_tag);
    if (tagIt != byTag.end())
        return static_cast<IBaseCSGrenadeProjectile*>(tagIt->second.ptr_for_return);
    auto* impl = new CBaseCSGrenadeProjectileImpl(this);
    byTag[&s_tag] = virtualhooks::EntityImplEntry(static_cast<IEntityInstance*>(impl), static_cast<IBaseCSGrenadeProjectile*>(impl));
    return impl;
}
inline IBaseCSGrenadeProjectile* IBaseCSGrenadeProjectile::FromRaw(CEntityInstance* p) { return p ? static_cast<CBaseCSGrenadeProjectile*>(p)->ToInterface() : nullptr; }
inline IBaseCSGrenadeProjectile* IBaseCSGrenadeProjectile::FromOriginal(CBaseCSGrenadeProjectile* p) { return p ? p->ToInterface() : nullptr; }
