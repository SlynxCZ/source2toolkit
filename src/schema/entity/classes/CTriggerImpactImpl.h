...
#ifndef _INCLUDE_CTRIGGERIMPACTIMPL_H
#define _INCLUDE_CTRIGGERIMPACTIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/ITriggerImpact.h"
...
public:
    explicit CTriggerImpactImpl(CTriggerImpact* p) : CTriggerMultipleImpl(p) {}

private:
    CTriggerImpact* Real() { return static_cast<CTriggerImpact*>(m_pReal); }
    CTriggerImpact* Real() const { return static_cast<CTriggerImpact*>(m_pReal); }

public:
    CTriggerImpact* GetOriginal() const override { return Real(); }
    float& Magnitude() override { return Real()->m_flMagnitude(); }
    void MagnitudeUpdated() override { Real()->m_flMagnitude.NetworkStateChanged(); }
    float& Noise() override { return Real()->m_flNoise(); }
    void NoiseUpdated() override { Real()->m_flNoise.NetworkStateChanged(); }
    float& Viewkick() override { return Real()->m_flViewkick(); }
