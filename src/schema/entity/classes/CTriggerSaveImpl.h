...
#ifndef _INCLUDE_CTRIGGERSAVEIMPL_H
#define _INCLUDE_CTRIGGERSAVEIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/ITriggerSave.h"
...
public:
    explicit CTriggerSaveImpl(CTriggerSave* p) : CBaseTriggerImpl(p) {}

private:
    CTriggerSave* Real() { return static_cast<CTriggerSave*>(m_pReal); }
    CTriggerSave* Real() const { return static_cast<CTriggerSave*>(m_pReal); }

public:
    CTriggerSave* GetOriginal() const override { return Real(); }
    ...
    float& DangerousTimer() override { return Real()->m_fDangerousTimer(); }
    void DangerousTimerUpdated() override { Real()->m_fDangerousTimer.NetworkStateChanged(); }
