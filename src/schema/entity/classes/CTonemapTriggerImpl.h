...
#ifndef _INCLUDE_CTONEMAPTRIGGERIMPL_H
#define _INCLUDE_CTONEMAPTRIGGERIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/ITonemapTrigger.h"
...
public:
    explicit CTonemapTriggerImpl(CTonemapTrigger* p) : CBaseTriggerImpl(p) {}

private:
    CTonemapTrigger* Real() { return static_cast<CTonemapTrigger*>(m_pReal); }
    CTonemapTrigger* Real() const { return static_cast<CTonemapTrigger*>(m_pReal); }

public:
    CTonemapTrigger* GetOriginal() const override { return Real(); }
    CUtlSymbolLarge& TonemapControllerName() override { return Real()->m_tonemapControllerName(); }
    void TonemapControllerNameUpdated() override { Real()->m_tonemapControllerName.NetworkStateChanged(); }
    CEntityHandle& TonemapController() override { return Real()->m_hTonemapController(); }
