...
#ifndef _INCLUDE_CENVMUZZLEFLASHIMPL_H
#define _INCLUDE_CENVMUZZLEFLASHIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/IEnvMuzzleFlash.h"
...
public:
    explicit CEnvMuzzleFlashImpl(CEnvMuzzleFlash* p) : CPointEntityImpl(p) {}

private:
    CEnvMuzzleFlash* Real() { return static_cast<CEnvMuzzleFlash*>(m_pReal); }
    CEnvMuzzleFlash* Real() const { return static_cast<CEnvMuzzleFlash*>(m_pReal); }

public:
    CEnvMuzzleFlash* GetOriginal() const override { return Real(); }
    float& Scale() override { return Real()->m_flScale(); }
    void ScaleUpdated() override { Real()->m_flScale.NetworkStateChanged(); }
    CUtlSymbolLarge& ParentAttachment() override { return Real()->m_iszParentAttachment(); }
