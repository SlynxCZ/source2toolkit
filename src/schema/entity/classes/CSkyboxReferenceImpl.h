...
#ifndef _INCLUDE_CSKYBOXREFERENCEIMPL_H
#define _INCLUDE_CSKYBOXREFERENCEIMPL_H

#pragma once

#include "source2toolkit/schema/entity/classes/ISkyboxReference.h"
...
public:
    explicit CSkyboxReferenceImpl(CSkyboxReference* p) : CBaseEntityImpl(p) {}

private:
    CSkyboxReference* Real() { return static_cast<CSkyboxReference*>(m_pReal); }
    CSkyboxReference* Real() const { return static_cast<CSkyboxReference*>(m_pReal); }

public:
    CSkyboxReference* GetOriginal() const override { return Real(); }
    WorldGroupId_t& WorldGroupId() override { return Real()->m_worldGroupId(); }
    void WorldGroupIdUpdated() override { Real()->m_worldGroupId.NetworkStateChanged(); }
    CHandle<CSkyCamera>& SkyCamera() override { return Real()->m_hSkyCamera(); }
